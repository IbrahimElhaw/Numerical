...
if __name__ == '__main__':
    pass
    X, Y, T, V, smoothed_V = get_preprocessed_data(11, 6)

    local_minimums, local_maximums = get_local_extrems(smoothed_V)
    show_min_max(X, Y, T, V, smoothed_V, local_minimums, local_maximums)

    for stroke in range(len(smoothed_V)):
        curr_T = T[stroke]
        curr_V = smoothed_V[stroke]
        ...
        for i in range(len(cur_local_max)):
            ...
            trimmed_velocity[curr_T > curr_T[used_local_min_index]] = 0
