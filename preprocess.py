...
def smooth_V(v, window_size=5, poly=2):
    smoothed_v = []
...
def get_preprocessed_data(person,
                          ...
                          smooth_poly=5,
                          nfs=15):
    X, Y, T = data.retrieve_data(person, charachter)
    X, Y = shift_to_origin(X, Y)
    V = calculate_velocity(X, Y, T)
    smoothed_V = smooth_V(V, smoothing_window, smooth_poly)
    ...
    return X, Y, T, V, smoothed_V


if __name__ == '__main__':
    X, Y, T, V, smoothed_V = get_preprocessed_data(3, 5)
